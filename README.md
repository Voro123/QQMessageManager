...
- AI 会被明确告知图片已过滤且自己看不到图片内容，不会假装看图
...
- AI 配置中的 Prompt 会作为代管核心规则注入到 AI system prompt 中，而不是普通聊天上下文
