...
每个说话风格按多个维度组织：身份与关系、性格与价值倾向、情绪表达、用词与语气、句式与聊天节奏、互动与回应习惯、口癖与非语言表达、边界与避免事项，并保留一个“其他规则”文本框容纳无法归类的补充。模型会自然融合这些维度，不会在回复中逐条复述。

程序内置“猫猫”风格：一只会说话、会卖萌但不过度堆砌口癖的黑猫，主人是水门。旧水门 Skill 文件已删除；旧自定义 Prompt 会在首次升级时迁移成一份可编辑的说话风格，旧水门选择会迁移到猫猫预设。

说话风格通过直接填写 QQ 号关联学习对象，不提供私聊对象下拉框，避免误解学习范围。目标 QQ 在私聊和各个 QQ 群中的新发言都会学习。每累计 N 句便调用当前聊天模型迭代一次八维风格，然后立即开始收集下一批。界面显示的是“当前批次样本 X / N”和累计迭代次数，而不是完成进度：学习会持续循环，没有终点，直到用户主动关闭。
