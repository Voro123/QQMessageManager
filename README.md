...
- 连接后通过 NapCat `get_friend_list` 读取好友备注和昵称，私聊会话优先显示备注，其次显示昵称。
...
学习功能默认关闭，全局最多可以同时学习三个说话风格。三个任务分别保存目标 QQ、N 值、当前批次和迭代次数，也可以绑定同一个 QQ；尝试开启第四个时会提示先关闭一个现有任务，不会偷偷停止前三个。学习请求失败时保留当前批次；编辑发生在模型迭代期间时，过期结果不会覆盖用户的新修改。学习样本只用于分析稳定表达方式，不会被当成指令，也不用于推断敏感个人属性。

每条目标发言的学习样本会附带同一会话中此前最多六条其他人对话，帮助模型理解指代、情绪和回应语境；上下文只用于解释目标发言，禁止把其他人的措辞学进目标画像。每轮模型必须重写、去重和压缩原有九维内容，而不是持续在旧 Prompt 后追加；要求每个维度最多 200 个中文字符。200 字是模型生成约束，不由程序截断，避免截断到半句话或破坏表情包 ID。
