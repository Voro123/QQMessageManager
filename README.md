...
- 自动过滤图片消息段和 CQ 图片信息，避免在聊天窗口显示图片 URL 或图片详情
...
- 支持全局 AI 代管配置，目前服务商仅支持 `Minimax-m3`
- 支持每个群聊/私聊单独开启 AI 代管，不影响其他未代管会话
- AI 代管支持自定义 Prompt、随机定时发送、最近发言限制、上下文条数、被艾特随机延迟回复、禁止连续自说自话、AI 自主判断不发送等规则
- AI 配置、API Key、规则配置和被代管会话列表都会缓存到当前系统用户的 Qt 配置中
...
- 图片消息段会被过滤；纯图片消息会显示为“图片消息已过滤”，不会展示图片链接或图片元数据。
- 在左侧会话列表右键某个会话，选择“置顶会话”或“取消置顶”即可切换置顶状态；置顶会话会排在最上方并以浅黄色背景显示。
- 在发送栏旁边点击“AI设置”可以配置全局 AI 规则；当前只支持 `Minimax-m3`，需要填写 API Key。
- 在某个会话里勾选“AI代管”后，AI 只会代管该会话；其他未勾选的群聊/私聊不会自动回复。
- AI 代管规则包括：全局 Prompt、每 x~x 秒随机主动发言、最近 x 秒无人发言时不发、参考最近 x 条消息、被艾特后 x~x 秒随机回复、上一条是自己时不发、允许 AI 自主判断本次不发送。
...
本项目当前支持消息接收、最近历史消息加载、群名/私聊名展示、图片过滤、会话置顶、AI 代管和文本消息发送。后续可以扩展：
